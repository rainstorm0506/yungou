...
<script type="text/javascript">
    function lxfEndtime(xsjx_time_shop, this_time) {
        ...
        lxfEndtime_setTimeout = window.setTimeout("lxfEndtime()", 1000);
                var endtime = <?php echo $item['xsjx_time']; ?>;
        ...
        $("#divAutoRTime").html("<b>限时揭晓</b><p>剩余时间：<em>" + days + "</em>天<em>" + CHour + "</em>时<em>" + CMinute + "</em>分<em>" + CSecond + "</em>秒</p>");
//        this.xsjx_time_shop.html("<b>限时揭晓</b><p>剩余时间：<em>" + days + "</em>天<em>" + CHour + "</em>时<em>" + CMinute + "</em>分<em>" + CSecond + "</em>秒</p>");
        ...
        if (endtime <= this.this_time) {
            ...
            $.post("<?php echo WEB_PATH; ?>/go/autolottery/autolottery_ret_install", {"shopid": {wc: $item['id']}}, function(data) {
                if (data == '-1') {
    ...
    $(function() {
        var temp1 = $("#timeall").attr("endtime");
        var temp2 = <?php echo time(); ?>;
        lxfEndtime(temp1,temp2); 
    });
</script>
<?php endif; ?>			
<!-- 限时揭晓end -->
...
<!--<p class="Pro_Detsingle" style="font-size:14px;">云购分区：<b style="color:#999;">分区一</b></p>-->
