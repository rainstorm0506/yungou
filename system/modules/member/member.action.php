...
class member extends admin {
    ...
    public function config() {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (!is_writable(dirname(__FILE__) . '/lib/user_reg_type.ini.php'))
                exit('user_reg_type.ini.php 没有写入权限!');
    ...
    public function member_fufen() {
        if (isset($_POST['submit'])) {
            ...
            if (!is_writable($path))
                _message('Please chmod  user_fufen.ini.php  to 0777 !');
...
HTML;
    ...
    public function recharge() {
        ...
        if (isset($_POST['sososubmit'])) {
            ...
            if (empty($chongzhi) || $chongzhi == '请选择充值来源')
                $content = " AND (`content`='充值' or `content`='管理员修改金额' or `content`='使用佣金充值到云购账户' or `content`='使用余额宝充值到云购账户')";
