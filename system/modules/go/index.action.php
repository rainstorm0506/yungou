...
class index extends base {
    ...
    public function init() {    
        ...
        //汽车配件cateid=6
        $car_goods = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid`=6 ORDER BY `id` DESC LIMIT 6");
        
        //服饰百货 cateid=12
        $cloth_goods = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid`=12 ORDER BY `id` DESC LIMIT 6");
        
        //母婴美妆 cateid=14
        $mother_goods = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid`=14 ORDER BY `id` DESC LIMIT 6");
        
        //生活用品 cateid=17
        $life_goods = $this->db->GetList("select * from `@#_shoplist` where `q_uid` is null and `cateid`=17 ORDER BY `id` DESC LIMIT 6");
